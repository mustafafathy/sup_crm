...
class Leads extends AdminController
{
    ...
    public function index($id = '', $category = '')
    {
        ...
        // Check if GDPR consent is required for leads
        ...
        // Pass the lead ID if it's available in the URL
        $data['leadid'] = $id;
        // Get category and search input from request

        // Load session library
        $this->load->library('session');

        // Get the search category from input
        $category = $this->input->get('searchCategory');
        // Store it in session if it's provided
        ...
        $data['categories'] = $this->leads_model->get_categories();
    ...
    public function saveCategorySession() {
        // Load session library
        $this->load->library('session');
    ...
    public function get_leads_by_category($category)
    {
        // Apply a category filter to the query
    ...
    {
        // Fetch all leads if no category is provided
        $query = $this->db->get('leads');
        return $query->result_array();
    }
    ...
    public function kanban()
    {
        ...
        $data['leads'] = $this->leads_model->get_filtered_leads([
            'category' => $category,
            'search' => $search
        ]);

        // Load the Kanban view with the filtered data
    ...
    public function lead($id = '')
    {
        ...
        if ($this->input->post()) {
            if (!$this->db->select('active')->where('id', $this->input->post('source'))->get(db_prefix() . 'leads_sources')->row()->active) {
                echo json_encode([
                    'alert_type' => 'warning',
                    'message' => '<font color="red">This category is currently inactive</font>',
                ]);
                die;
            }
