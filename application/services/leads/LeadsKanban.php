...
class LeadsKanban extends AbstractKanban
{
    ...
    {
        parent::__construct($status, $category);
            $this->category = $category; // Save category
    		$this->status = $status;
            $this->applyCategoryFilter(); // Ensure this is called before executing the query
    ...
    protected function applySearchQuery($q): self
    {
        if (!startsWith($q, '#')) {
            ...
            $this->ci->db->where('(' . db_prefix() . 'leads.name LIKE "%' . $q . '%" ESCAPE \'!\' OR ' .
                db_prefix() . 'leads_sources.name LIKE "%' . $q . '%" ESCAPE \'!\' OR ' .
                db_prefix() . 'leads.email LIKE "%' . $q . '%" ESCAPE \'!\' OR ' .
                db_prefix() . 'leads.phonenumber LIKE "%' . $q . '%" ESCAPE \'!\' OR ' .
                db_prefix() . 'leads.phonesearch LIKE "%' . $q . '%" ESCAPE \'!\' OR ' .
                db_prefix() . 'leads.company LIKE "%' . $q . '%" ESCAPE \'!\' OR ' .
    ...
    protected function applyCategoryFilter(): self
    {
        if (!empty($this->category)) {
            $this->ci->db->where(db_prefix() . 'leads.category', $this->category); // Make sure this line targets the correct category column
        }
        return $this;
    }
    ...
    {
        $this->ci->db->select(db_prefix() . 'leads.title, ' . db_prefix() . 'leads.website, ' .
            db_prefix() . 'leads.lead_value, ' . db_prefix() . 'leads.address, ' .
            db_prefix() . 'leads.city, ' . db_prefix() . 'leads.state, ' .
            db_prefix() . 'leads.country, ' . db_prefix() . 'leads.zip, ' .
            db_prefix() . 'leads.name as lead_name,' . db_prefix() . 'leads_sources.name as source_name,' .
            db_prefix() . 'leads.id as id,' . db_prefix() . 'leads.assigned,' .
            db_prefix() . 'leads.email,' . db_prefix() . 'leads.phonenumber,' .
            db_prefix() . 'leads.company,' . db_prefix() . 'leads.dateadded,' .
            db_prefix() . 'leads.status,' . db_prefix() . 'leads.lastcontact, ' .
            '(SELECT COUNT(*) FROM ' . db_prefix() . 'clients WHERE leadid=' . db_prefix() . 'leads.id) as is_lead_client, ' .
            ...
            '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' .
            db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' .
            db_prefix() . 'leads.id and rel_type="lead" ORDER by tag_order ASC) as tags');
