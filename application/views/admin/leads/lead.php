<?php defined('BASEPATH') or exit('No direct script access allowed'); ?>
    <input type="hidden" name="category" id="hiddenCategoryInput" value="">
...
function getCategory($key='')
{
    ...
    // Check if the provided key exists in the categories array
    if (array_key_exists($key, $categories)) {
        return $categories[$key]; // Return the category name
    } else {
        return 'Category not found'; // Return a default message if the key is not found
    }
}
            if (isset($lead)) {
    if (!empty($lead->name)) {
    ...
    echo '#' . $lead->id . ' - ' . $name;
echo '<p id="categoryDisplay">' . htmlspecialchars(getCategory($lead->category)) . '</p>';
} else {
        

// Echo the title and the category in a formatted way
echo _l('add_new', _l('lead_lowercase'));
echo '<p id="categoryDisplay">' . htmlspecialchars(getCategory($category)) . '</p>';

// Retrieve the category from the post data if available, or fallback to a default (if not posted)
// if $category = 1
