...
$rules = [
    ...
    App_table_filter::new('phonesearch', 'TextRule')->label('Phone-No'),
    App_table_filter::new('category', 'TextRule')->label('Account-Category'),
...
return App_table::find('leads')
    ->outputUsing(function ($params) use ($statuses, $category) {
        ...
        foreach ($custom_fields as $key => $field) {
            if ($field['id'] == 7) {
                array_push($aColumns, 'ctable_' . $key . '.value as ' . 'cvalue_' . $key);
                break;
            }
        }
        ...
        $aColumns = array_merge($aColumns, [
            ...
            db_prefix() . 'leads.phonesearch as phonesearch',
            db_prefix() . 'leads.phonenumber as phonenumber',
            db_prefix() . 'leads.category as category',
        ...
        foreach ($custom_fields as $key => $field) {
            ...
            array_push($customFieldsColumns, [$selectAs, $field['id']]);
            if ($field['id'] != 7)
        ...
        if (!empty($category)) {
            ...
            // You can also add a condition here to handle cases where no category is present.
        }

        // Retrieve filters from rules if they exist
        $filtersWhere = $this->getWhereFromRules();
        if ($filtersWhere) {
            $where[] = $filtersWhere;
        }

        // Check if the user has permissions to view the leads
        if (staff_cant('view', 'leads')) {
            $where[] = 'AND (assigned = ' . get_staff_user_id() . ' OR addedfrom = ' . get_staff_user_id() . ' OR is_public = 1)';
        }

        // Now $where contains all the necessary conditions
        ...
        foreach ($rResult as $aRow) {
            ...
            // Custom owner's name field add value
            foreach ($customFieldsColumns as $customFieldColumn) {
                if ($customFieldColumn[1] == 7) {
                    $row[] = $aRow[$customFieldColumn[0]];
                    break;
                }
            }
            ...
            $nameRow = '<a ' . $hrefAttr . '>' . e($full_name) . '</a>';
            ...
            $buttons = '<div class="row-options">';
            $buttons .= '<a ' . $hrefAttr . '>' . _l('view') . '</a>';
            ...
            if (!$locked) {
                $buttons .= ' | <a href="' . admin_url('leads/index/' . $aRow['id'] . '?edit=true') . '" onclick="init_lead(' . $aRow['id'] . ', true);return false;">' . _l('edit') . '</a>';
            }

            if ($aRow['addedfrom'] == get_staff_user_id() || $has_permission_delete) {
                $buttons .= ' | <a href="' . admin_url('leads/delete/' . $aRow['id']) . '" class="_delete text-danger">' . _l('delete') . '</a>';
            }
            $buttons .= '</div>';

            $row[] = $nameRow . $assignedOutput . $buttons;
            ...
            foreach ($customFieldsColumns as $customFieldColumn) {
                ...
                $row[] = (strpos($customFieldColumn[0], 'date_picker_') !== false ? _d($aRow[$customFieldColumn[0]]) : $aRow[$customFieldColumn[0]]);
            }
