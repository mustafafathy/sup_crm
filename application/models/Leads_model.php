...
class Leads_model extends App_Model
{
     ...
     * @return mixed
     */
    public function get($id = '', $where = [], $category = '')
    {
        // Select fields from leads, status, and sources tables
        $this->db->select('*,' . db_prefix() . 'leads.name, ' . db_prefix() . 'leads.category, ' . db_prefix() . 'leads.id,' . db_prefix() . 'leads_status.name as status_name,' . db_prefix() . 'leads_sources.name as source_name');
        ...
        // If a specific lead ID is passed, retrieve that lead
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'leads.id', $id);
            $lead = $this->db->get(db_prefix() . 'leads')->row();

            // If the lead is found, fetch its related data
            if ($lead) {
                ...
                // Attachments and public URL
                $lead->attachments = $this->get_lead_attachments($id);
                $lead->public_url = leads_public_url($id);
            }

            return $lead; // Return the single lead data
        }

        // If a category is provided, apply the category filter
        if (!empty($category)) {
            $this->db->where(db_prefix() . 'leads.category', $category);
        }

        // Return the result set of leads (either filtered by category or all leads)
      // Apply the sorting: latest entries based on 'last_lead_status', 'lastcontact', and 'dateadded'
    $this->db->order_by('last_lead_status', 'DESC');
    ...
    public function get_filtered_leads($filters = [])
    {
        ...
        if (isset($filters['search']) && !empty($filters['search'])) {
            ...
            $this->db->or_like('company', $filters['search']);
    ...
    // Reuse the remove_specific_chars function if needed
    function remove_specific_chars($string)
    {
    ...
    public function add($data)
    {
        ...
        if (!isset($data['phonesearch']) || isset($data['phonesearch']) && $data['phonesearch'] == '') {
            $data['phonesearch'] = $this->remove_specific_chars($data['phonenumber']);
        ...
        if (isset($data['category'])) {
            ...
            if ($category) {
                ...
                // Set a default value if no session data is available
                $data['category'] = '1'; // Replace 'default_category' as needed
            }
        }
    ...
    public function get_categories($id = '', $where = [], $db = false)
    {
        if ($db == true) {
            $this->db->select('id, name');  // Select id and name of the categories
            $query = $this->db->get('categories');  // Replace 'categories' with the actual table name

            // Initialize an empty array to store the categories
            $categories = [];

            // Loop through the query result and build the categories array
            foreach ($query->result() as $row) {
                $categories[$row->id] = $row->name;  // Use id as key and name as value
            }
            ...
            // If an ID is provided, return the category for that ID
            if (!empty($id) && isset($categories[$id])) {
                return $categories[$id];
            }

            // Filter categories based on conditions in the $where array
            if (!empty($where)) {
                $filteredCategories = [];
                ...
                foreach ($categories as $key => $category) {
                    ...
                    foreach ($where as $conditionKey => $conditionValue) {
                        // Add filtering logic (for example, category name matching or other conditions)
                        if ($conditionKey == 'name' && stripos($category, $conditionValue) === false) {
                            $match = false;
                            break;
                        }
                    }
