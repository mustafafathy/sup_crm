...
class Reports_model extends App_Model
{
    ...
    public function leads_staff_report()
    {
        ...
        if ($this->input->post()) {
            ...
            $from_date = to_sql_date($this->input->post('date_from'));
            $to_date   = to_sql_date($this->input->post('date_to'));
        }
        $chart = [
            'labels'   => [],
            'datasets' => [
                ...
                [
                    'label'           => "Lead Weight",
                    'backgroundColor' => 'rgba(255,109,10,0.4)',
                    'borderColor'     => '#FF5733',
                    'borderWidth'     => 1,
                    'tension'         => false,
                    'data'            => [],
        ...
        foreach ($staff as $member) {
            ...
            if (!isset($to_date) && !isset($from_date)) {
				$today = date('Y-m-d');
				$this->db->where('(CASE WHEN assigned = 0 THEN addedfrom = ' . $member['staffid'] . ' ELSE assigned = ' . $member['staffid'] . ' END) AND status = 1 AND ((last_status_change IS NOT NULL AND DATE(last_status_change) = "' . $today . '") OR DATE(dateadded) = "' . $today . '")', '', false);
				$total_rows_converted = $this->db->count_all_results(db_prefix() . 'leads');

            	$this->db->from(db_prefix() . 'leads');
				$this->db->where('addedfrom', $member['staffid']);
				$this->db->group_start();
				$this->db->where('(last_status_change IS NOT NULL AND (status = 1 OR status = 8) AND DATE(last_status_change) = "' . $today . '")', '', false);
				$this->db->or_where('DATE(dateadded) = "' . $today . '"', '', false);
				$this->db->group_end();
				$total_rows_created = $this->db->count_all_results();

				$this->db->where('(CASE WHEN assigned = 0 THEN addedfrom = ' . $member['staffid'] . ' ELSE assigned = ' . $member['staffid'] . ' END) AND lost = 1 AND ((last_status_change IS NOT NULL AND DATE(last_status_change) = "' . $today . '") OR DATE(dateadded) = "' . $today . '")', '', false);
				...
				$this->db->select('SUM(CASE 
										...
										ELSE 1
									END) as weighted_score', false);
				$this->db->from(db_prefix() . 'leads l');
				$this->db->join(db_prefix() . 'customfieldsvalues cf', 'l.id = cf.relid AND cf.fieldid = 4', 'left');
				$this->db->group_start();
				$this->db->where('(l.last_status_change IS NOT NULL AND (l.status = 1 OR l.status = 8) AND DATE(l.last_status_change) = "' . $today . '")', '', false);
				$this->db->or_where('DATE(l.dateadded) = "' . $today . '"', '', false);
				$this->db->group_end();
				$this->db->where('l.addedfrom', (int)$member['staffid']);
				$this->db->group_start();
				$this->db->where('l.status', 1);
				$this->db->or_where('l.status', 2);
				$this->db->or_where('l.status', 8);
				$this->db->group_end();

				$query = $this->db->get();
				$total_rows_lead_weight = $query->row()->weighted_score;
            } else {
                $sql = 'SELECT COUNT(' . db_prefix() . 'leads.id) as total FROM ' . db_prefix() . 'leads WHERE ((last_status_change IS NOT NULL AND DATE(last_status_change) BETWEEN "' . $this->db->escape_str($from_date) . '" AND "' . $this->db->escape_str($to_date) . '") OR (DATE(dateadded) BETWEEN "' . $this->db->escape_str($from_date) . '" AND "' . $this->db->escape_str($to_date) . '")) AND status = 1 AND (CASE WHEN assigned = 0 THEN addedfrom = ' . $member['staffid'] . ' ELSE assigned=' . $member['staffid'] . ' END)';
            	$total_rows_converted = $this->db->query($sql)->row()->total;

                $sql = 'SELECT COUNT(' . db_prefix() . 'leads.id) as total FROM ' . db_prefix() . 'leads WHERE ((last_status_change IS NOT NULL AND (status = 1 OR status = 8) AND DATE(last_status_change) BETWEEN "' . $this->db->escape_str($from_date) . '" AND "' . $this->db->escape_str($to_date) . '") OR (DATE(dateadded) BETWEEN "' . $this->db->escape_str($from_date) . '" AND "' . $this->db->escape_str($to_date) . '")) AND addedfrom=' . $member['staffid'];
                $total_rows_created = $this->db->query($sql)->row()->total;

                $sql = 'SELECT COUNT(' . db_prefix() . 'leads.id) as total FROM ' . db_prefix() . 'leads WHERE ((last_status_change IS NOT NULL AND (status = 1 OR status = 8) AND DATE(last_status_change) BETWEEN "' . $this->db->escape_str($from_date) . '" AND "' . $this->db->escape_str($to_date) . '") OR (DATE(dateadded) BETWEEN "' . $this->db->escape_str($from_date) . '" AND "' . $this->db->escape_str($to_date) . '")) AND lost = 1 AND (CASE WHEN assigned=0 THEN addedfrom=' . $member['staffid'] . ' ELSE assigned=' . $member['staffid'] . ' END)';
				...
				LEFT JOIN `' . db_prefix() . 'customfieldsvalues` cf ON l.id = cf.relid AND cf.fieldid = 4 
				WHERE ((l.last_status_change IS NOT NULL AND (l.status = 1 OR l.status = 8) AND DATE(l.last_status_change) BETWEEN "' . $this->db->escape_str($from_date) . '" AND "' . $this->db->escape_str($to_date) . '") OR (DATE(l.dateadded) BETWEEN "' . $this->db->escape_str($from_date) . '" AND "' . $this->db->escape_str($to_date) . '"))
				AND l.addedfrom = ' . (int)$member['staffid'] . ' AND (l.status = 1 OR l.status = 2 OR l.status = 8)';
                $total_rows_lead_weight = $this->db->query($sql)->row()->weighted_score;
            }
            ...
            array_push($chart['datasets'][3]['data'], $total_rows_lead_weight);
        }
