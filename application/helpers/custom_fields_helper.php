 ...
 * @return mixed
 */
function render_custom_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [], $pipeline = null)
{
    ...
    if ($pipeline) {
        $CI->db->where('pipeline', intval($pipeline)); // Cast to int if stored as an integer

    }// Check if the pipeline exists in the session and apply it in the WHERE clause
    if ($selectedPipeline) {
        // Ensure the value is treated as an integer (if stored as an integer in the DB)
        $CI->db->where('pipeline', intval($selectedPipeline));
    } 
    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            if ($field['id'] == 4 && staff_cant('view', 'leads'))
                continue;
