...
$(function () {
  ...
  // On change event for category select dropdown
  $('#searchCategorySelect').on('change', function () {
    // Get the selected category value (the text, not the ID)
    var selectedCategory = $(this).find('option:selected').text();

    // Perform a search on the DataTable with the selected category
    table.search(selectedCategory).draw();

    // Focus on the search input after the change (optional)
    $("div.dataTables_filter input").focus();
  });
