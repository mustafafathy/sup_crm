...
function setRulesInGlobal() {
  app.dtFilters[props.id] = {
    match_type: localMatchType.value,
    rules: builderRules.value.map((r) => {
      // Check if the rule is for phone numbers and apply formatting
      if (r.type === 'phone') {
        // const countryCode = r.country_code || '1'; // Use a default country code if needed
        return {
          id: r.id,
          value: formatPhoneNumber(r.value),  // Format the phone number
          has_dynamic_value: r.has_dynamic_value === true,
          operator: r.operator,
          type: r.type,
        };
      ...
      // Default case: return the rule as-is for other types
      return {
        id: r.id,
        value: r.value,
        has_dynamic_value: r.has_dynamic_value === true,
...
function applyFilters() {
  setRulesInGlobal();
  $(tableSelector.value).DataTable().ajax.reload(); // Reload table with new filtered values
}
...
function formatPhoneNumber(phone) {
  // Remove all non-numeric characters (e.g., dashes, spaces, brackets)
